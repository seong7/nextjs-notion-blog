...
export function replaceCover() {
  const wrapper = document.getElementsByClassName('notion-page-scroller')[0];
  const image = document.getElementsByClassName('notion-page-cover')[0];
  const canvas = document.createElement('canvas');
  canvas.id = 'swarm';

  if (wrapper && canvas) {
    if (image) wrapper.replaceChild(canvas, image);
    fn1(window);
  }
}
...
export function cleanUp() {
  ...
  window.removeEventListener('load', gLoadListener);
  gResize = undefined;
  gLoadListener = undefined;
}
...
function fn1(root) {
  ('use strict');
  console.log('fn1 ㄷㅏ시 호출 됨 ');
  const Vector3D = function Vector3D(x, y, z) {
    this.set(x, y, z);
  };
  const v3dp = Vector3D.prototype;
  ...
  v3dp.eq = function (other) {
    return (
      other instanceof Vector3D && this.x === other.x && this.y === other.y && this.z === other.z
    );
  };
  ...
  const MouseMonitor = function (element) {
  ...
  const Particle = function Particle(generator, bounds, rctx, mon) {
  ...
  pp2.step = function () {
    ...
    const a = this.r.random() * Math.Tau;
  ...
  */
  ...
  function loadListener() {
    const rctx = new SmallPRNG(+new Date()); // random generator, see ref
    const p = new Perlin(); // simplex noise generator
    const canvas = document.getElementById('swarm');
    const context = canvas.getContext('2d');
    console.log(context);
    ...
    f1.add(settings, 'particleNum', 1000, 15000)
      ...
      .onChange(function () {
        if (settings.particleNum < particles.length) {
          const toDelete = particles.length - settings.particleNum;
          particles.splice(particles.length - toDelete, toDelete);
        } else {
          for (let i = particles.length; i < settings.particleNum; i += 1) {
    ...
    resize = function () {
      // resize the canvas
      canvas.width = width = bounds.x = window.innerWidth * 0.7;
      canvas.height = height = bounds.y = window.innerHeight * 0.25;
    ...
    // console.log('stats ::', statsUI);
    statsUI.style.display = 'none';
    const dgUI = document.getElementsByClassName('dg ac')[0];
    // console.log('dg ::', dgUI);
    dgUI.style.display = 'none';

    // generate a few particles
    for (let i = 0; i < settings.particleNum; i += 1) {
      particles.push(new Particle(p, bounds, rctx, monitor));
    }
